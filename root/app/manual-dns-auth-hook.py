...
ACMEDNS_URL = "http://localhost:9020"
...
if __name__ == "__main__":
    ...
    if FORCE_REGISTER or not account:
        ...
        with open('/var/lib/acme-dns/cname.txt', 'w') as file:
            print("Wrote authorization details to /var/lib/acme-dns/cname.txt. Please consult that file for next steps.")
            file.writelines([msg.format(cname), "\nPlease delete this file when you are done, the validation process will wait for you to do this to continue"])

    # Update the TXT record in acme-dns instance
    client.update_txt_record(account, VALIDATION_TOKEN)
    while os.path.exists('/var/lib/acme-dns/cname.txt'):
        sleep(1)
